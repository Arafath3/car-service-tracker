import type { ServiceInterval, Vehicle, ServiceRecord } from '@/types';

export const CAR_SERVICE_INTERVALS: ServiceInterval[] = [
  { serviceType: 'Oil & Filter Change', intervalKm: 10000, description: 'Engine oil and filter replacement', intervalMonths: 12 },
  ...
  { serviceType: 'Brake Fluid', intervalKm: 40000, description: 'Replace brake fluid', intervalMonths: 24 },
  { serviceType: 'Coolant Flush', intervalKm: 60000, description: 'Flush and replace engine coolant', intervalMonths: 48 },
...
export const MOTORBIKE_SERVICE_INTERVALS: ServiceInterval[] = [
  { serviceType: 'Oil & Filter Change', intervalKm: 6000, description: 'Engine oil and filter replacement', intervalMonths: 12 },
  ...
  { serviceType: 'Coolant', intervalKm: 30000, description: 'Replace coolant (liquid-cooled bikes)', intervalMonths: 24 },
];

export const getBaseIntervals = (vehicleType: 'car' | 'motorbike'): ServiceInterval[] =>
...
export interface ServiceStatus {
  ...
  source: 'default' | 'sticker' | 'custom-interval' | 'custom-service' | 'estimated';
...
const getEffectiveLastService = (
  ...
  if (
    ...
    const estDate = new Date(Date.now() - monthsAgo * 30 * DAY_MS).toISOString();
    return {
...
export const calculateServiceStatuses = (
  ...
  return intervals.map((interval) => {
    ...
    const effective = getEffectiveLastService(vehicle, interval.serviceType, services);
    ...
    if (lastReal?.nextDueOdometer != null) {
      ...
      if (interval.intervalMonths) {
        ...
        nextDueDate = new Date(base + interval.intervalMonths * 30 * DAY_MS).toISOString();
      }
      if (effective.source === 'estimated') source = 'estimated';
      else if (vehicle.customServiceTypes?.some((c) => c.serviceType === interval.serviceType)) {
    ...
    if (!effective) {
      ...
      const kmDueSoon = kmRemaining > 0 && kmRemaining <= interval.intervalKm * 0.1;
      const dateDueSoon = daysRemaining != null && daysRemaining > 0 && daysRemaining <= 30;
    ...
    const kmProgress = effective
      ? Math.max(0, Math.min(1,
          (vehicle.currentOdometer - effective.odometer) / interval.intervalKm
        ))
      : 0;
    const dateProgress =
      nextDueDate && effective && interval.intervalMonths
        ...
        : 0;
    const progressPercent = Math.round(Math.max(kmProgress, dateProgress) * 100);
...
export const finalizeEstimation = (
  ...
  const dailyKm = totalKmDuringWindow > 0 ? totalKmDuringWindow / daysElapsed : 30; // fallback
...
import type { VehicleEstimation } from '@/types';